...
#-----------------------------Build network encoder based on VGG16 network and load the trained VGG16 weights-----------------------------------------
       #Layer 1
        self.conv1_1 = self.conv_layer_ELU(bgr, "conv1_1") #Build Convolution layer and load weights
        self.conv1_2 = self.conv_layer_ELU(self.conv1_1, "conv1_2")#Build Convolution layer +Relu and load weights
        self.pool1 = self.max_pool(self.conv1_2, 'pool1') #Max Pooling
       # Layer 2
        self.conv2_1 = self.conv_layer_ELU(self.pool1, "conv2_1")
        self.conv2_2 = self.conv_layer_ELU(self.conv2_1, "conv2_2")
        self.pool2 = self.max_pool(self.conv2_2, 'pool2')
        # Layer 3
        self.conv3_1 = self.conv_layer_ELU(self.pool2, "conv3_1")
        self.conv3_2 = self.conv_layer_ELU(self.conv3_1, "conv3_2")
        self.conv3_3 = self.conv_layer_ELU(self.conv3_2, "conv3_3")
        self.pool3 = self.max_pool(self.conv3_3, 'pool3')
        # Layer 4
        self.conv4_1 = self.conv_layer_ELU(self.pool3, "conv4_1")
        self.conv4_2 = self.conv_layer_ELU(self.conv4_1, "conv4_2")
        self.conv4_3 = self.conv_layer_ELU(self.conv4_2, "conv4_3")
        self.pool4 = self.max_pool(self.conv4_3, 'pool4')
        # Layer 5
        self.conv5_1 = self.conv_layer_ELU(self.pool4, "conv5_1")
        self.conv5_2 = self.conv_layer_ELU(self.conv5_1, "conv5_2")
        self.conv5_3 = self.conv_layer_ELU(self.conv5_2, "conv5_3")
...
##-----------------------Build Net Fully connvolutional layers------------------------------------------------------------------------------------
        ...
        self.elu6 = tf.nn.elu(self.conv6, name="elu6")
        # if FLAGS.debug: utils.add_activation_summary(relu6)
        self.elu_dropout6 = tf.nn.dropout(self.elu6,keep_prob=keep_prob)  # Apply dropout for traning need to be added only for training

        W7 = utils.weight_variable([1, 1, 4096, 4096], name="W7")  # 1X1 Convloution
        b7 = utils.bias_variable([4096], name="b7")
        self.conv7 = utils.conv2d_basic(self.elu_dropout6, W7, b7)  # 1X1 Convloution
        self.elu7 = tf.nn.elu(self.conv7, name="elu7")
        # if FLAGS.debug: utils.add_activation_summary(relu7)
        self.elu_dropout7 = tf.nn.dropout(self.elu7, keep_prob=keep_prob)  # Another dropout need to be used only for training
        ...
        self.conv8 = utils.conv2d_basic(self.elu_dropout7, W8, b8)
...
#-------------------------------------Build Decoder --------------------------------------------------------------------------------------------------
     ...
     #--------------------Transform  probability vectors to label maps-----------------------------------------------------------------
        ...
        self.Pred1 = tf.expand_dims(self.Pred, dim = 3)
        
        self.probabilities = tf.nn.softmax(self.Prob)
...
############################################################################################################################################################
    def conv_layer_ELU(self, bottom, name):
        with tf.variable_scope(name):
            filt = self.get_conv_filter(name)
