# -*- coding: utf-8 -*-
"""
Created on Sat Dec  2 15:28:06 2017
...
def relaxprecision(original, classified, relax):
    ...
    w_lim, h_lim = imclassified.size

    truepositive = 0
    for i in range(0, h_lim, 1):
        for j in range(0, w_lim, 1):
            prediction_val = Pixclassified[i*w_lim+j]
...
def relaxrecall(original, classified, relax):
    ...
    for i in range(0, h_lim, 1):
        for j in range(0, w_lim, 1):
            label_val = Pixoriginal[i*w_lim+j]
